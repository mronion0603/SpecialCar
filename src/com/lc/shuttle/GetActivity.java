...
public class GetActivity extends Activity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.usecardate:{
			timepWindow = new TimePopupWindow(GetActivity.this,itemsOnClick);
			timepWindow.showAsDropDown(originview, 0, 0); 		
		}	break;
		case R.id.AirportAddress:{
			airportWindow = new AddressPopupWindow(GetActivity.this,itemOnClick2,groups2,"附近机场");//实例化AddressPopupWindow
			airportWindow.showAsDropDown(originview, 0, 0); //显示窗口
		}	break;
		case R.id.Star:
		{	menuWindow = new AddressPopupWindow(GetActivity.this,itemOnClick,groups1);//实例化AddressPopupWindow
			menuWindow.showAsDropDown(originview, 0, 0); //显示窗口
			if(groups1.size()<=0 && mSwitch){
				Toast.makeText(GetActivity.this,"暂无收藏地址,请到地址管理页面添加", Toast.LENGTH_LONG).show();
			}
		}	break;
		case R.id.Search:{
			...
			if(startTime.equals("请输入接机时间")){
				...
				Toast.makeText(getApplication(), "请输入下车地点", Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(getApplication(), "请选择车型", Toast.LENGTH_SHORT).show();
				 ...
				 if(MySharePreference.getStringValue(getApplication(), MySharePreference.USER_TYPE).equals("1")){
			            ...
			            addShuttleNet.seteLatitude(String.valueOf(elat));
			            addShuttleNet.seteLongitude(String.valueOf(elont));
			            addShuttleNet.setEndAddress(tvendaddress.getText().toString());
			            ...
			            addShuttleNet.setsLatitude(String.valueOf(slat));
			            addShuttleNet.setsLongitude(String.valueOf(slont));
			            addShuttleNet.setStartAddress(airportstr);
			            ...
			            addShuttleNet.setServiceTypeId("1");
		...
		{	Intent intent4 = new Intent();
			intent4.setClass(GetActivity.this,SelectCarActivity.class);
			startActivityForResult(intent4, REQUSET_SELECTCAR);
		}	break;
		case R.id.rldiscount:
		{	Intent intent = new Intent();
			intent.setClass(GetActivity.this,Discount2Activity.class);
            ...
            switch(msg.what) {
	            ...
	            case Global.ADDRESS_END_MESSAGE:{
	            	...
	            	if(etAirport.getText().toString().equals("请输入机场")){
	            		...
	            		//System.out.println(String.valueOf(elat)+","+String.valueOf(elont));
	            ...
	            case Global.CHOOSECITY: {
	            	...
	            	if(tvendaddress.getText().toString().equals("请输入下车地点")|tvendaddress.getText().toString().equals("获取地址失败")
    ...
    private void parseInner(String str)throws Exception{ 
    	...
    	int result = jsonobj.getInt("ResultCode");
   	    if(result==Global.SUCCESS){	
			 ...
			 intent2.setClass(GetActivity.this,SendDealActivity.class);
